...
with gr.Blocks(title=VERSION, theme=gr.themes.Soft(primary_hue="purple", secondary_hue="blue"), css=CSS, js=JS_SCRIPT) as GXAI:
    ...
    with gr.Tab("🎗️BgRemoverLite"):
        ...
        with gr.Row():
            image_input_dir = gr.Textbox(
                label="Enter dir", placeholder="Enter dir like this: D:\Python\GXAI")
...
##################################################################################################################################
    ...
    with gr.Tab("⤴️Upscaler"):
        ...
        with gr.Row():
            upsc_batch_input = gr.Textbox(label="Enter dir", placeholder="Enter dir like this: D:\Python\GXAI")
...
##################################################################################################################################
    ...
    with gr.Tab("🔎Image Analyzer"):
        ...
        with gr.Row():
            detector_input = gr.Textbox(label="Enter dir", placeholder="Enter dir like this: D:\Python\GXAI")
...
##################################################################################################################################
    ...
    with gr.Tab("🎥Video Analyzer"):
        ...
        with gr.Row():
            video_dir = gr.Textbox(label="Videos dir", placeholder="Enter dir like this: D:\Python\GXAI")
...
##################################################################################################################################            
    ...
    with gr.Tab("🤖AI Detector"):
        ...
        with gr.Row():
            aid_input_batch = gr.Textbox(label="Enter dir", placeholder="Enter dir like this: D:\Python\GXAI")
...
##################################################################################################################################
    ...
    with gr.Tab("⚙️Settings"):
        ...
        with gr.Row():
            settings_start_in_browser = gr.Checkbox(value=config.inbrowser, label="Enable GXAI starting in browser")
        with gr.Row():
            settings_share_gradio = gr.Checkbox(value=config.share_gradio, label="Enable GXAI starting with share link")
        with gr.Row():
            settings_clear_on_start = gr.Checkbox(value=config.clear_on_start, label="Enable clear all outputs on GXAI start")
        with gr.Row():
        ...
        with gr.Row():
            ...
            btn_refresh = gr.Button(value="🔁 Restart GXAI")
            
##################################################################################################################################

    rembg_button.click(gx.BgRemoverLite, inputs=image_input, outputs=image_output)
    rembg_batch_button.click(gx.BgRemoverLiteBatch, inputs=image_input_dir, outputs=image_output_dir)
    clearp_bgr_button.click(gx.BgRemoverLite_Clear)

    detector_button.click(gx.NSFW_Detector, inputs=detector_input, outputs=detector_output)
    detector_clear_button.click(gx.NSFWDetector_Clear)
    
    upsc_button.click(gx.Upscaler, inputs=[upsc_image_input, scale_factor, model_ups], outputs=upsc_image_output)
    upsc_button_batch.click(gx.Upscaler_batch, inputs=[upsc_batch_input, scale_factor_batch, model_ups_batch], outputs=upsc_batch_output)
    clear_upsc_button.click(gx.Upscaler_batch_Clear)
    upsc_clear_cache.click(gx.CODC_clear_app)
    
    spc_button.click(gx.ImageAnalyzer, inputs=[file_spc, clip_checked, clip_chunk_size], outputs=spc_output)

    Vspc_button.click(gx.VideoAnalyzer, inputs=file_Vspc, outputs=Vspc_output)
    start_dir_videos.click(gx.VideoAnalyzerBatch, inputs=[video_dir, vbth_slider, threshold_Vspc_slider], outputs=bth_Vspc_output)   
    clear_videos.click(gx.VideoAnalyzerBatch_Clear)   
    
    promptgen_button.click(gx.PromptGenetator, inputs=[prompt_input, pg_prompts, pg_max_length, randomize_temp], outputs=promptgen_output)
    
    aid_single_button.click(gx.AiDetector_single, inputs=aid_input_single, outputs=aid_output_single)
    aid_batch_button.click(gx.AiDetector_batch, inputs=aid_input_batch, outputs=aid_output_batch)
    aid_clear_button.click(gx.AID_Clear)
    
    clear_all_button.click(config.clear_all)
    
    check_torch.click(config.check_gpu)
    
    settings_save.click(config.save_config_gr, 
    inputs=[
        settings_debug_mode, 
        settings_start_in_browser, 
        settings_share_gradio, 
    ...
    tts_button.click(gx.silero_tts, inputs=[tts_input, tts_lang, tts_speakers, tts_rate], outputs=tts_audio)
    tts_clear.click(gx.tts_clear)
    
    refresh_button.click(
    refresh_gallery, 
    outputs=[
        gallery_rembg_outputs,
...
GXAI.launch(inbrowser=config.inbrowser, share=config.share_gradio)